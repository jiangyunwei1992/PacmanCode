...
public class KNNAlgorithm 
{
	...
	private static int votePrediction(Map<Double,List<Integer>> classification)
	{
		...
		{
			double key = it.next();
			List<Integer> cls = classification.get(key);
			// if the number of candidates is more than count, then select the first k
			if(cls.size()>=count)
			{
...
//		System.out.println("start time = "+(new Date()));
		...
		for(int i=0;i<testX.length;i++)
		{
			...
			for(int j=0;j<trainX.length;j++)
			{
				...
				if(!classification.containsKey(distance))
				{
					List<Integer> labels = new ArrayList<Integer>();
					labels.add(trainY[j]);
					classification.put(distance, labels);
				}else
				{
					classification.get(distance).add(trainY[j]);
				}
			}
